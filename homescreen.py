import pygame
import os, sys
from sparkle import SparkleParticle 
from utils import board_matrix, q, MAX_TIME_LIMIT, WIDTH, HEIGHT, white, grey, black, blue, red, green, yellow, clock
import cv2

class HomeScreen:
    def __init__(self):
        self.screen = pygame.display.set_mode((WIDTH, HEIGHT))
        
        # Load video from the correct path
        self.video = cv2.VideoCapture("Larawan/home.mp4")
        
        self.sparkles = [SparkleParticle() for _ in range(30)]
        self.font = pygame.font.Font("Fonts/Fredoka-Regular.ttf", 40)

    def get_video_frame(self):
        ret, frame = self.video.read()
        if not ret or frame is None:
            self.video.set(cv2.CAP_PROP_POS_FRAMES, 0)  # Loop video
            ret, frame = self.video.read()
            if not ret or frame is None:
                return pygame.Surface((WIDTH, HEIGHT))  # Fallback blank surface

        frame = cv2.resize(frame, (WIDTH, HEIGHT))
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        return pygame.surfarray.make_surface(frame.swapaxes(0, 1))

    def show(self):
        running = True
        frame_count = 0

        while running:
            # Draw video frame as background
            video_frame = self.get_video_frame()
            self.screen.blit(video_frame, (0, 0))

            frame_count += 1

            for sparkle in self.sparkles:
                sparkle.update()
                sparkle.draw(self.screen)

            pygame.display.update()

            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    self.video.release()
                    pygame.quit()
                    sys.exit()
                if event.type == pygame.MOUSEBUTTONDOWN:
                    return  # Start the game
